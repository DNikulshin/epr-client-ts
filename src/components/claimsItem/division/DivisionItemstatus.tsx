...
export const DivisionItemStatus = ({id, itemId, itemName, isEdit}: IDivision) => {
  ...
  const deleteDivision =
    useCallback(async (id: number | string | undefined, itemId: number | string | undefined, userId: string | number | null) => {
      if (itemId) {
        const status = await divisionDelete(id, itemId, userId)
        if (status === 'OK')
          setIsDelete(true)
      } else {
        setIsDelete(false)
      ...
      >
        {!isDelete && isEdit &&
          <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" fill="currentColor"
               ...
               viewBox="0 0 16 16"
               onClick={() => id ?  deleteDivision(id, itemId, userId) : false}
          >
            <path
