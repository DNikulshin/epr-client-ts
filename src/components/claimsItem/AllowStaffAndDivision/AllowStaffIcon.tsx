import { useCallback, useState } from 'react'
import { useDataStore } from '../../../store/data-store/data-store.ts'

interface AllowStaffIconProps {
  id: number
  itemId: number | string | undefined
}

export const AllowStaffIcon = ({id ,itemId}: AllowStaffIconProps) => {
  const employeeAdd = useDataStore(state => state.employeeAdd)
  const employeeDelete = useDataStore(state => state.employeeDelete)
  const [addEmployee, setAddEmployee] = useState(false)


  const addStaffHandler = useCallback(async () => {
   const status =  await employeeAdd(id, itemId )
   if(status === 'OK') {
     setAddEmployee(true)
   }

  }, [employeeAdd, itemId, id])

  const deleteStaffHandler = useCallback(async () => {
    const status =  await employeeDelete(id, itemId, )
    if(status === 'OK') {
      setAddEmployee(false)
    }

  }, [employeeDelete, id, itemId])

  return (
   <>
     {!addEmployee ? <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" fill="currentColor"
                          className="bi bi-person-add me-2 text-success btn-hover" viewBox="0 0 16 16"
                          onClick={addStaffHandler}
       >
         <path
           d="M12.5 16a3.5 3.5 0 1 0 0-7 3.5 3.5 0 0 0 0 7m.5-5v1h1a.5.5 0 0 1 0 1h-1v1a.5.5 0 0 1-1 0v-1h-1a.5.5 0 0 1 0-1h1v-1a.5.5 0 0 1 1 0m-2-6a3 3 0 1 1-6 0 3 3 0 0 1 6 0M8 7a2 2 0 1 0 0-4 2 2 0 0 0 0 4" />
         <path
           d="M8.256 14a4.5 4.5 0 0 1-.229-1.004H3c.001-.246.154-.986.832-1.664C4.484 10.68 5.711 10 8 10q.39 0 .74.025c.226-.341.496-.65.804-.918Q8.844 9.002 8 9c-5 0-6 3-6 4s1 1 1 1z" />
       </svg>
       :
       <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" fill="currentColor"
            className="bi bi-person-dash me-2 text-danger" viewBox="0 0 16 16"
            onClick={deleteStaffHandler}
       >
         <path
           d="M12.5 16a3.5 3.5 0 1 0 0-7 3.5 3.5 0 0 0 0 7M11 12h3a.5.5 0 0 1 0 1h-3a.5.5 0 0 1 0-1m0-7a3 3 0 1 1-6 0 3 3 0 0 1 6 0M8 7a2 2 0 1 0 0-4 2 2 0 0 0 0 4" />
         <path
           d="M8.256 14a4.5 4.5 0 0 1-.229-1.004H3c.001-.246.154-.986.832-1.664C4.484 10.68 5.711 10 8 10q.39 0 .74.025c.226-.341.496-.65.804-.918Q8.844 9.002 8 9c-5 0-6 3-6 4s1 1 1 1z" />
       </svg>}
   </>


  )
}